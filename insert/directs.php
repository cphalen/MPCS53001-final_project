...
$DirectorName = htmlspecialchars($_GET["name"]);

?>

<br>
<div class="container">
    <form action="../api/insertDirects.php" method="post">
        <?php
            ...
            } elseif ($status == "noSuchDirectorName") {
                echo '<div class="alert alert-danger" role="alert"> There does not appear to be any director matching the input. Please check your inputs and try again! </div>';
            } elseif ($status == "success") {
                echo '<div class="alert alert-success" role="alert"> Your data was successfully inserted into the database! </div>';
            }
        ?>
        <h3>
            Insert a new row in the directs in table with all of the below information
        </h3>
        <small>
